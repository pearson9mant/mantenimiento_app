...
def sembrar_planificacion_legionella(fecha_inicio):
    ...
    for _, punto in puntos_df.iterrows():
        ...
        for tarea in tareas_por_tipo_punto(
            tipo_punto,
            tipo_control_punto
        ):
...
def asegurar_columnas_plano_legionella():
    ...
    try:
        ejecutar("""
            ALTER TABLE legionella_puntos
            ADD COLUMN numero_terminales INTEGER DEFAULT 1
        """)
    except Exception:
        pass
...
"""
    ...
    with st.expander("📋 Matriz normativa / cobertura documental", expanded=False):
        ...
        elif normativa.get("color") == "amarillo":
            st.warning(f"{normativa.get('estado')} · {normativa.get('score')}%")
        else:
            st.success(f"{normativa.get('estado')} · {normativa.get('score')}%")
        ...
        if normativa.get("recomendaciones"):
            ...
            for r in normativa.get("recomendaciones", []):
                st.markdown(f"• {r}")

        st.info(normativa.get("nota", ""))

    with st.expander("🌡️ Temperaturas y planificación", expanded=False):
        st.markdown(f"**Temperaturas:** {temperaturas.get('estado')} · {temperaturas.get('score')}%")
        st.caption(temperaturas.get("mensaje", ""))

        for motivo in temperaturas.get("motivos", []):
            st.markdown(f"• {motivo}")

        st.markdown("---")

        st.markdown(f"**Planificación:** {planificacion.get('estado')} · {planificacion.get('score')}%")
        st.caption(planificacion.get("mensaje", ""))
    ...
    with st.expander("📈 Estabilidad de puntos", expanded=False):
        if not estabilidad:
            st.info("Todavía no hay histórico suficiente para valorar estabilidad.")
        else:
            for p in estabilidad:
                st.markdown(
                    f"**{p.get('punto', '-')}** · "
                    f"{p.get('estado', '-')} · {p.get('score', 0)}%"
                )
                st.caption(p.get("mensaje", ""))
...
def pantalla_legionella():
        ...
        if df_plan.empty:
            ...
            for _, row in df_filtrado.iterrows():
                ...
                es_extraordinaria_finalizada = (
                    ...
                    and sin_proxima_fecha
                )

                if es_extraordinaria_finalizada:
                    titulo = (
                        f"✅ FINALIZADA · {icono_plan} · "
                        f"{row['centro']} · {row['edificio']} · "
                        f"{row['punto']} · {row['tarea']} · {estado}"
                    )
                else:
                    proxima_txt = (
                        ""
                        if sin_proxima_fecha
                        else str(row.get("proxima_fecha") or "").strip()
                    )

                    titulo = (
                        f"{row['estado_control']} · {icono_plan} · "
                        f"{row['centro']} · {row['edificio']} · "
                        f"{row['punto']} · {row['tarea']} · "
                        f"{proxima_txt or 'Sin fecha'} · {estado}"
                    )
    ...
    with tab3:
        ...
        try:
            ...
            st.error(f"No se han podido cargar los puntos: {e}")
            puntos_admin = pd.DataFrame()

        if puntos_admin.empty:
            st.info("No hay puntos registrados. Crea el primer punto arriba.")
        else:
