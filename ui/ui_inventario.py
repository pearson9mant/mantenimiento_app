...
def pantalla_inventario():
    ...
    # El texto libre actúa como búsqueda global. Si quedó una categoría
    # seleccionada de una consulta anterior, no bloquea los resultados.
    # Centro y edificio sí se respetan porque son filtros físicos útiles.
    categoria_consulta = (
        "Todas"
        if str(filtro_texto or "").strip()
        else filtro_categoria
    )

    if str(filtro_texto or "").strip() and filtro_categoria != "Todas":
        st.caption(
            f"🔎 Búsqueda global activa: la categoría '{filtro_categoria}' "
            f"se ignora mientras escribes."
        )

    materiales = obtener_materiales_inventario_ligero(
        filtro_texto=filtro_texto,
        filtro_categoria=categoria_consulta,
